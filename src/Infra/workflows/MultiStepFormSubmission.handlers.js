...
import { getForm, dummyFx } from '@/Infra/endpoints/form.endpoints'

export const handleGetFormClientSide = async ({ serializer = defaultFormSerializer, showSuccess = dummyFx, showError = console.error } = {}) => {
    const user = await supabase.auth.getUser()
    if (!user?.data?.user || user?.error) return { error: user?.error, data: serializer(user?.data?.user) }
    const { data, error } = await getForm({ user, ctx, serializer, showSuccess, showError })
    return { data, error }
}
export const handleGetFormServerSide = async ({ ctx = null, serializer = defaultFormSerializer, showSuccess = dummyFx, showError = console.error } = {}) => {
